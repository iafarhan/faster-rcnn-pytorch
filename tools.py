...
def coord_trans(bbox, w_pixel, h_pixel, w_amap=7, h_amap=7, mode='a2p'):
  ...
  if mode == 'p2a':
    ...
    resized_bbox[:, :, [0, 2]] /= width_ratio.view(-1, 1, 1) #for each image in a batch,for each object in image
    resized_bbox[:, :, [1, 3]] /= height_ratio.view(-1, 1, 1) #converted to 3d to ensure the division is applied to required dim only9
  else:
    # activation to pixel
    width_ratio = w_pixel * 1. / w_amap
    height_ratio = h_pixel * 1. / h_amap
...
def DetectionInference(detector, data_loader, dataset, idx_to_class, thresh=0.8, nms_thresh=0.3, output_dir=None, dtype=torch.float32, device='cpu'):
  ...
  for iter_num, data_batch in enumerate(data_loader):
    ...
    for idx in range(batch_size):
      ...
      print(final_conf_scores[idx].shape)
      ...
      if output_dir is not None:
        ...
        utils.vis.detection_visualizer(img, idx_to_class, boxes[idx][:valid_box], resized_proposals)
